function GoogleLoginV2(config) {
	...
	*/
	...
	this.prepare = function () {
		...
		// If user is logging into lounge, the ddc accessToken is ignored.
		...
		window.localStorage.setItem("loungeLoginWithGoogleV2", true);
		window.loginRedirector.redirectToSocialLogin(this.config.clientId, "google");
		return;
	};
	...
	this.authorizeCode = function(authCode) {
		...
		promise.then(function(response){
			...
			window.localStorage.removeItem("loungeLoginWithGoogleV2");
			window.localStorage.removeItem("googleAuthCodeLounge");
